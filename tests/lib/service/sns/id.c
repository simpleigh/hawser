...
START_TEST(test_write_static_memory)
{
	...
	ck_assert_int_eq(HAWSER_OK, sns_id_from_string(&id, VALID_IDS[_i]));
	ck_assert_int_eq(HAWSER_OK, sns_string_from_id(&string, &id));
...
{
	SNS_ID id;
	char aString[SNS_ID_BYTES];
	char *szString = aString;

	ck_assert_int_eq(HAWSER_OK, sns_id_from_string(&id, VALID_IDS[_i]));
	ck_assert_int_eq(HAWSER_OK, sns_string_from_id(&szString, &id));
...
{
	SNS_ID id;
	ck_assert_int_eq(HAWSER_NULL, sns_string_from_id(NULL, &id));
}
END_TEST
...
{
	char *string;
	ck_assert_int_eq(HAWSER_NULL, sns_string_from_id(&string, NULL));
}
END_TEST
